...
mount(function () {
    if (auth()->user()) {
        ...
        $this->total = $this->subtotal[0]->total + $this->taxes + ($this->subtotal[0]->total * 14 / 100);
    }
});
...
<div class="max-w-screen-xl mx-auto min-h-screen">
    <div class="flex flex-col md:flex-row gap-8 size-full">
        <div class="size-full flex flex-col gap-y-8 py-6 px-3 md:py-12 md:px-6">
            @foreach ($cart->cartItems as $item)
            <div class="flex items-center space-x-6" wire:key="{{$item->id}}">
                <div class="h-28 w-28 flex-shrink-0 overflow-hidden rounded-lg bg-secondary">
                    <img src="{{$item->productItem->product->image_url}}" alt="{{$item->productItem->product->name}}"
                        class="h-full w-full object-cover">
                </div>
                <div class="flex-grow">
                    <div class="flex items-center justify-between">
                        <h3 class="font-semibold">{{$item->productItem->product->name}}</h3>
                        <span class="font-bold">${{ $item->productItem->price * $item->quantity }}</span>
                    </div>
                    <p class="text-gray-600">${{$item->productItem->price}} x {{ $item->quantity }}</p>
                    <p class="text-gray-600 capitalize font-medium">{{$item->productItem->size}}</p>
                    <p class="text-gray-600 capitalize">{{$item->productItem->pizza_type}}</p>
                </div>
            </div>
            @endforeach
            ...
            <div class="flex flex-col gap-y-4 mt-8">
                <div class="flex items-center justify-between">
                    <p class="text-muted-foreground font-medium">Subtotal</p>
                    <span class="font-medium">${{ $subtotal[0]->total }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <p class="text-muted-foreground font-medium">Taxes</p>
                    <span class="font-medium">${{ $subtotal[0]->total * 14 / 100 }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <p class="text-muted-foreground font-medium">Delivery</p>
        ...
        <div class="size-full py-6 px-3 md:py-12 md:px-6">
            <button
                class="flex w-full items-center p-2 text-base font-bold text-white rounded-lg bg-black hover:bg-gray-900 group hover:shadow">
